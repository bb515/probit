    ...
    if dataset in datasets["benchmark"]:
        ...
        for i, varphi in enumerate(varphis):
            ...
            log_p_pseudo_marginals, log_p_prior = hyper_sampler.tmp_compute_marginal(
                    theta, indices, reparameterised=False)
            log_p_pseudo_marginalss.append(log_p_pseudo_marginals)
        ...
        plt.plot(varphis, log_p_pseudo_marginals_ms + log_p_pseudo_marginals_std, '--b')
        plt.plot(varphis, log_p_pseudo_marginals_ms - log_p_pseudo_marginals_std, '--b')
        plt.plot(varphis, log_priors, '--g')
        plt.savefig("tmp0.png")
        plt.show()

        log_p_priors = log_p_priors + np.log(varphis_step)
        max_log_p_priors = np.max(log_p_priors)
        log_sum_exp = max_log_p_priors + np.log(np.sum(np.exp(log_p_priors - max_log_p_priors)))
        p_priors = np.exp(log_p_priors - log_sum_exp)

        max_log_p_pseudo_marginals = np.max(log_p_pseudo_marginals_ms)
        log_sum_exp = max_log_p_pseudo_marginals + np.log(
            np.sum(np.exp(log_p_pseudo_marginals_ms - max_log_p_pseudo_marginals)))
        ...
        log_p_pseudo_marginalss = log_p_pseudo_marginalss - np.log(varphis_step).reshape(-1, 1)
        ...
        p_pseudo_marginals = np.exp(log_p_pseudo_marginalss - log_sum_exp)
