...
g_star = -1. * np.ones(3)
n0, g0, patches = ax[0].hist(gamma_samples[:, 0], 20, density="probability", histtype='stepfilled')
n1, g1, patches = ax[1].hist(gamma_samples[:, 1], 20, density="probability", histtype='stepfilled')
n2, g2, patches = ax[2].hist(gamma_samples[:, 2], 20, density="probability", histtype='stepfilled')
#n2, g3, patches = ax[2].hist(gamma_samples[:, 3], 20, density="probability", histtype='stepfilled')
g_star[0] = g0[np.argmax(n0)]
g_star[1] = g1[np.argmax(n1)]
g_star[2] = g2[np.argmax(n2)]
ax[0].axvline(g_star[0], color='k', label=r"Maximum $\gamma_1$")
ax[1].axvline(g_star[1], color='k', label=r"Maximum $\gamma_2$")
ax[2].axvline(g_star[2], color='k', label=r"Maximum $\gamma_3$")
ax[0].set_xlabel(r"$\gamma_1$", fontsize=16)
ax[1].set_xlabel(r"$\gamma_2$", fontsize=16)
ax[2].set_xlabel(r"$\gamma_3$", fontsize=16)
ax[0].legend()
ax[1].legend()
ax[2].legend()
...
plt.scatter(X[np.where(t == 1)], m_tilde[np.where(t == 1)], color=colors[0], label=r"$t={}$".format(1))
plt.scatter(X[np.where(t == 2)], m_tilde[np.where(t == 2)], color=colors[1], label=r"$t={}$".format(2))
plt.scatter(X[np.where(t == 3)], m_tilde[np.where(t == 3)], color=colors[2], label=r"$t={}$".format(3))
plt.xlabel(r"$x$", fontsize=16)
plt.ylabel(r"$\tilde{m}$", fontsize=16)
plt.title("GP regression posterior sample mean mbar, plotted against x")
plt.show()

plt.scatter(X[np.where(t == 1)], y_tilde[np.where(t == 1)], color=colors[0], label=r"$t={}$".format(1))
plt.scatter(X[np.where(t == 2)], y_tilde[np.where(t == 2)], color=colors[1], label=r"$t={}$".format(2))
plt.scatter(X[np.where(t == 3)], y_tilde[np.where(t == 3)], color=colors[2], label=r"$t={}$".format(3))
plt.xlabel(r"$x$", fontsize=16)
...
plt.stackplot(x, Z.T,
              labels=(
                  r"$p(t=0|x, X, t)$", r"$p(t=1|x, X, t)$", r"$p(t=2|x, X, t)$", r"$p(t=3|x, X, t)$", r"$p(t=4|x, X, t)$"),
              colors=(
                  colors[3], colors[0], colors[1], colors[2], colors[4])
              )
plt.legend()
val = 0.5  # this is the value where you want the data to appear on the y-axis.
plt.scatter(X[np.where(t == 1)], np.zeros_like(X[np.where(t == 1)]) + val, facecolors=colors[0], edgecolors='white')
plt.scatter(X[np.where(t == 2)], np.zeros_like(X[np.where(t == 2)]) + val, facecolors=colors[1], edgecolors='white')
plt.scatter(X[np.where(t == 3)], np.zeros_like(X[np.where(t == 3)]) + val, facecolors=colors[2], edgecolors='white')
plt.show()
