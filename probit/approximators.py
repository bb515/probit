...
class VBGP(Approximator):
    ...
    def _theta(
            self, posterior_mean, theta_hyperparameters, n_samples=10,
            vectorised=False):
        """
                ...
                for d in range(self.D):
                    if trainables[self.J + 1][d]:
                        if numerical_stability is True:
                            ...
                            gx[self.J + 1 + d] = one + two
                            if verbose:
                                print("one", one)
                                print("two", two)
                                print("gx = {}".format(gx[self.J + 1 + d]))
            else:
                if trainables[self.J + 1]:
                    if numerical_stability is True:
                        # Update gx[-1], the partial derivative of the lower bound
...
class LaplaceGP(Approximator):
    ...
    def compute_weights(
        ...
        cov = solve_triangular(L_cov, L_covT_inv, lower=False)
        log_det_cov = -2 * np.sum(np.log(np.diag(L_cov)))
        return (weight, precision, w1, w2, g1, g2, v1, v2, q1, q2, L_cov, cov,
            Z, log_det_cov)
