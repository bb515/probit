...
class GibbsMultinomialGPOrdered(Sampler):
    """
    ...
    def _sample_initiate(self, m_0, y_0, gamma_0):
        ...
        if np.shape(gamma_0)[0] == self.K - 2:  # not including infinity cutpoints at 0 and K, so K = len(gamma_0) + 1
            ...
            if gamma_0[0] is not np.NANF:  # not including negative infinity cutpoint
                if gamma_0[-1] not in [np.infty, np.inf]:
        ...
        for _ in trange(first_step, first_step + steps,
            ...
            for k in range(1, self.K - 1):
                ...
                if indeces:
                    ...
                    locs[k - 1] = gamma[k - 1]
                print('k = ', k, ', locs = ', locs, ' , uppers = ', uppers)
            ...
            for n, m_n in enumerate(m):  # i in range N
                ...
                k_true = self.t_train[n]
                # Initiate yi at 0
                y_n = -1.0  # this is a trick for the next line
